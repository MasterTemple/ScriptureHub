...
var lightTheme = true;
var previousAccentColor = getComputedStyle(
...
async function getPassage(book, chapter, translation) {
  return new Promise((resolve, reject) => {
    fetch(
      `https://raw.githubusercontent.com/MasterTemple/ScriptureHub/main/BibleGateway/translations/json/${book}/${chapter}/${translation}.json`
    ).then((response) => {
      try {
        // response.json()
        let res = response.json();
...
async function getInterlinearVerse(book, chapter, verse, num) {
  return new Promise(async (resolve, reject) => {
    fetch(
      `https://raw.githubusercontent.com/MasterTemple/ScriptureHub/main/BibleHub/json/interlinear/${book}/${chapter}/${verse}.json`
    ).then(async (response) => {
      try {
        // response.json()
        let res = await response.json();
...
async function searchVerse() {
  ...
  let { book, chapter, start_verse, end_verse, givenTranslation } = [
    ...verse.matchAll(
      /(?<book>\d? ?\S*) (?<chapter>\d{1,3}):?(?<start_verse>\d{1,3})?-?(?<end_verse>\d{1,3})? ?(?<givenTranslation>[A-z0-9]+)?/gim
    ),
  ]?.[0]?.groups;
  ...
  if (lowerBook === "psalm") {
    ...
    lowerBook === "song of solomon" ||
    lowerBook === "songs of solomon" ||
    lowerBook === "song of songs"
  ) {
    book = "Songs";
  }
    ...
    if (key !== rightContent) {
      if (key === "interlinear") {
        if (!end_verse) {
          apiData[key] = [
            await get(
              `https://raw.githubusercontent.com/MasterTemple/ScriptureHub/main/BibleHub/json/interlinear/${book}/${chapter}/${start_verse}.json`
            ),
          ];
        } else {
          apiData[key] = await get(
            `https://raw.githubusercontent.com/MasterTemple/ScriptureHub/main/BibleHub/json/interlinear/${book}/${chapter}.json`
          );

          apiData[key] = Object.entries(apiData[key])
            .filter(
              ([k, v]) => parseInt(k) >= start_verse && parseInt(k) <= end_verse
            )
            .map(([k, v]) => v);
        }
      }
      if (key === "commentary") {
        apiData[key] = await get(
          `https://raw.githubusercontent.com/MasterTemple/ScriptureHub/main/BibleHub/json/commentaries/${book}/${chapter}/${start_verse}.json`
        );
      }
      if (key === "context") {
        apiData[key] = await get(
          `https://raw.githubusercontent.com/MasterTemple/ScriptureHub/main/BibleGateway/translations/json/${book}/${chapter}/${primaryTranslation}.json`
        );
      }
    }
  });
...
async function updateVerseContent(
  book,
  chapter,
  start_verse,
  end_verse,
  ...
  let translations = [
    primaryTranslation,
    ...availableTranslations.filter((f) => f !== primaryTranslation),
  ];
  ...
  for (const i in translations) {
    ...
    let card_header = getVerseCardHeader(
      book,
      chapter,
      verseRange,
      translation
    ...
    let card_text = getVerseCardText(
      passage,
      start_verse,
      end_verse,
      translation
...
async function updateRightContent() {
  return new Promise(async (resolve, reject) => {
    ...
    if (document.getElementById("strongs")) {
      document
        .getElementById("main")
        .removeChild(document.getElementById("strongs"));
    }
...
async function interlinearExpandStrongs(lang, num) {
  ...
  // closes it
  if (
    document.querySelector(`#strongs-${lang}-${num} > svg`).style.transform ===
    "rotate(180deg)"
  ) {
  ...
  // switching one
    ...
    }
    {
      ...
      document.querySelector(`#strongs-${lang}-${num} > svg`).style.transform =
        "rotate(180deg)";
      document
        .querySelector(`#strongs-${lang}-${num} > svg`)
        .classList.add("strongs-open");
      ...
      document.querySelector("main > .first").style.display = "none";
      let data = await get(
        `https://raw.githubusercontent.com/MasterTemple/ScriptureHub/main/BibleHub/json/strongs/${lang}/${num}.json`
      );
      console.log(data);
      data.forEach(
        (
          ...
          let og_verseBeforeChild = document.createTextNode(
            og_before.join(" ")
          );
...
function commentaryDropDown(c) {
  ...
  if (
    document.querySelector(`.cmt-${c} > svg`).style.transform ===
    "rotate(90deg)"
  ) {
...
function interlinearVerseDropdown(c) {
  ...
  if (
    document.querySelector(
      `#interlinear-verse-heading-${c} > .interlinear-verse-container > svg`
...
function copyVerse(translation) {
  ...
  verseData.forEach(({ ...e }, c) => {
    ...
    if (e.num) {
      ...
      if (
        e.num >= globalVerses.start_verse &&
        e.num <= globalVerses.end_verse
      ) {
        if (settings.numbers) {
    ...
    // its a header
      ...
      if (
        verseData[c + 1]?.num >= globalVerses.start_verse &&
        verseData[c + 1]?.num <= globalVerses.end_verse
      ) {
        if (settings.headers) {
          if (settings.discord) {
            textToCopy += `**${e.header}**`;
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  initialVerse = initialVerse.replace(
    "https://github.com/MasterTemple/ScriptureHub/main/index.html?",
    ""
  ...
  references = await get(
    "https://raw.githubusercontent.com/MasterTemple/ScriptureHub/main/refs.json"
  );
  document.getElementById("color-picker").value = getComputedStyle(
    document.documentElement
  ).getPropertyValue("--accent-color");
  // let translationHTML = ""
...
document.addEventListener("keyup", function (event) {
  // console.log(event);
  if (event.key === "Enter") {
    document.getElementById("search").value = document
      .getElementById("search")
      .value.replace(/^./g, (m) => m.toUpperCase())
      .replace(/(?<=^\d? ?[A-z]+)(?=\d+:)/g, " ");
    // verse[0] = verse[0].toUpperCase()
...
document.getElementById("confirm-button").addEventListener("click", () => {
  ...
  previousAccentColor = getComputedStyle(
    document.documentElement
  ).getPropertyValue("--accent-color");
